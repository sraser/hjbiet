...
def get_ma(ticker):
    ...
    ma20 = dfma['close'].rolling(12).mean().iloc[-1]
    return ma5, ma20
...
krw = get_balance("KRW")
time.sleep(0.5)
btcma5, btcma20 = get_ma("KRW-BTC")
time.sleep(0.5)
ethma5, ethma20= get_ma("KRW-ETH")
bot.sendMessage(chat_id=chat_id, text="재접속")
startmsg = "HJ KRW : %d BTC : %.8f ETH : %.8f" % (krw , btc_ini, eth_ini)
...
while True:
    try:
        ...
        btcma5, btcma20= get_ma("KRW-BTC")
        time.sleep(0.1)
        btcma20b = btcma20 * 1.001
        btcma20s = btcma20 * 0.998
        ...
        ethma5, ethma20= get_ma("KRW-ETH")
        time.sleep(0.1)
        ethma20b = ethma20 * 1.001
        ethma20s = ethma20 * 0.998

        krw = get_balance("KRW")

        if btc_state == 0 and btcma5 > btcma20b:
            btcbuycall = 0
            if eth > ethfee :
            ...
            if krw > 5000 and btc < btcfee:
                ...
                btc_state = 1
                msg = "HJ BTC 매수 = %.2f" % (btc_buy_price)
                bot.sendMessage(chat_id=chat_id, text=msg)
            time.sleep(0.2)

        if eth_state == 0 and ethma5 > ethma20b:
            ethbuycall = 0
            if btc > btcfee :
        ...
        print(e)
        bot.sendMessage(chat_id=chat_id, text="HJ e")
        time.sleep(1)
